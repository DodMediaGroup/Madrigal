...
class PlanosController extends Controller
{
        ...
        if(isset($_POST['Apartment']))
        {
            ...
            if($model->validate(null, false)){
                if($_FILES['logo']['size'] > 0){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['logo'], 500, 'planos/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'planos/', $model->image, 350, 350);
                    }
                }
                else{
                    $model->addError('image', 'Es necesario agregar una imagen!!!');
                    $errors = true;
                }
        ...
        if(isset($_POST['Apartment']))
        {
            ...
            if($model->validate(null, false)){
                if($_FILES['logo']['size'] > 0){
                    ...
                    $uploadLogo = MyMethods::uploadImage($_FILES['logo'], 200, 'slide/');
                    ...
                    if(!$uploadLogo['status']){
                        ...
                        MyMethods::resizeImage($server.'slide/', $model->image, 300 , 300);
                         ...
                         @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/planos/".$currentImagen);
                        @unlink($_SERVER['DOCUMENT_ROOT'].Yii::app()->request->baseUrl."/images/planos/350x350/".$currentImagen);
                    }
                }
            }
            if(!$errors && $model->save()){
    ...
    public function actionAdmin()
    {
        ...
        $planos = Apartment::model()->findAll(array('condition'=>'t.status != 2', 'order'=>'t.id DESC'));
    ...
    {
         $model=Apartment::model()->findByAttributes(array('id'=>$id), array('condition'=>'t.status != 2'));
        if($model===null)
            throw new CHttpException(404,'The requested page does not exist.');
        return $model;
    }
